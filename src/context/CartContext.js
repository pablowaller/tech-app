...
export const CartComponentContext = (props) => {
  ...
  const addItems = (itemAgregado) => {
    setSubTotal(
      subtotal + calculatePrice(itemAgregado.item.price, itemAgregado.quantity)
    );
  ...
  const removeItems = (id) => {
    ...
    setSubTotal(
      subtotal - calculatePrice(itemToRemove.item.price, itemToRemove.quantity)
    );
  ...
  return (
    <CartContext.Provider
      value={{ addItems, removeItems, clear, subtotal, itemsQty, items }}
    >
      {props.children}
    </CartContext.Provider>
  );
